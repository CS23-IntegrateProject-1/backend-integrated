...
export const createReservation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const selectedTable = await findSuitableTable(getAvailableTablesResponse);
    if (isResponse) {
      if (
        !selectedTable ||
        selectedTable === null ||
        ...
        isResponse = false;
        return res.status(400).json({ error: "No suitable tables available." });
      }
...
export const createTable = async (req: Request, res: Response) => {
  try {
    ...
    const { information, tableTypeDetailId, tablenumber, branchId } = req.body;
...
export const getCountPerDay = async (req: Request, res: Response) => {
  try {
    ...
    const reservationIdsArray = transaction.map((reserve) => reserve.reserveId);
...
export const createOfflineReservation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const selectedTable = await findSuitableTable(offlineAvailabilityResponse);
    if (isResponse) {
      if (
        !selectedTable ||
        selectedTable === null ||
